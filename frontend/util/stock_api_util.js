...
export const getStockNews = (name) => {
  const date = new Date;
  const threeWeeksAgoDate = new Date(date.getTime() - (60*60*24*21*1000));
  ...
  const threeWeeksAgo = `${threeWeeksAgoDate.getFullYear()}-${threeWeeksAgoDate.getMonth() + 1}-${threeWeeksAgoDate.getDate()}`;

  return $.ajax({
    method: "GET",
    url: `https://newsapi.org/v2/everything?q=${name}&language=en&from=${threeWeeksAgo}&to=${today}&sortBy=popular&pageSize=10&apiKey=9345391bfd414fef81f23c2939844ad5&sources=${sources}`
  })
}
